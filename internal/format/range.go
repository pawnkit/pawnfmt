...
func (formatter *Formatter) FormatRange(source []byte, start, end int) (RangeResult, error) {
	...
	if parsed.HasParseErrors() && formatter.config.ParseMode == config.ParseModeStrict {
		return RangeResult{}, parseDiagnostic(source, parsed, "source")
	}
	if parsed.Root == nil {
		return RangeResult{}, errors.New("source has no syntax tree")
	}
	...
	// Include sorting is a multi-node transformation and therefore is not
	// applied by single-unit range formatting.
	...
	if formattedParsed.Root == nil {
		return RangeResult{}, errors.New("formatted syntax tree no longer matches the selected range")
	}
	formattedNode := correspondingRangeNode(parsed.Root, formattedParsed.Root, node)
	if formattedNode == nil {
		return RangeResult{}, errors.New("could not locate selected syntax in formatted output")
	}
	...
	if verified.HasParseErrors() && !parsed.HasParseErrors() {
		return RangeResult{}, parseDiagnostic(out, verified, "range-formatted output")
	}
